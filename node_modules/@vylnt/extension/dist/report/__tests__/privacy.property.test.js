...
const urlArb = fc.oneof(fc.constant("https://example.com/page"), fc.constant("https://internal.corp/admin"), fc.constant("http://localhost:3000/app"), fc.webUrl());
/** Generate a blocklist that is guaranteed to contain the given URL */
function blocklistContaining(url) {
...
const emptyBlocklistArb = fc.constant([]);
...
describe("Property 21: Blocklist privacy enforcement", () => {
    it("matchesBlocklist returns true when URL is in the blocklist (exact match)", () => {
        fc.assert(fc.property(urlArb, blocklistContaining("https://example.com/page"), (url, blocklist) => {
    ...
    it("matchesBlocklist returns false when blocklist is empty", () => {
        fc.assert(fc.property(urlArb, emptyBlocklistArb, (url, blocklist) => {
            expect(matchesBlocklist(url, blocklist)).toBe(false);
        }), { numRuns: 100 });
    });
    it("matchesBlocklist supports glob wildcard patterns", () => {
        fc.assert(fc.property(fc.constantFrom("https://internal.corp/admin", "https://internal.corp/settings", "https://internal.corp/users"), (url) => {
    ...
    it("blocklisted URL produces zero API calls when submitReport is called after blocklist check", async () => {
        await fc.assert(fc.asyncProperty(urlArb, blocklistContaining("https://example.com/page"), async (_url, blocklist) => {
...
//# sourceMappingURL=privacy.property.test.js.map