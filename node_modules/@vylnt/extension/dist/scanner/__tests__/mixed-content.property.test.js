...
const domSnapshotArb = fc.record({
...
describe("Property 4: Mixed content finding completeness", () => {
    it("produces a finding for every HTTP resource on an HTTPS page", () => {
        fc.assert(fc.property(domSnapshotArb, (snapshot) => {
            ...
            for (const httpUrl of allHttpUrls) {
                if (!findings.some((f) => f.type === "mixed_content" && f.affectedResource === httpUrl))
                    return false;
            }
    ...
    it("assigns severity high to scripts and medium to all others", () => {
        fc.assert(fc.property(fc.array(httpUrlArb, { minLength: 1, maxLength: 3 }), fc.array(httpUrlArb, { minLength: 1, maxLength: 3 }), (scripts, links) => {
            const snapshot = {
                scripts, links, images: [], iframes: [], media: [], formActions: [],
            };
            const findings = detectMixedContent("https://secure.example.com/", snapshot);
            for (const f of findings) {
                if (f.type !== "mixed_content")
                    continue;
                const rt = f.details.resourceType;
                if (rt === "script" && f.severity !== "high")
                    return false;
                if (rt !== "script" && f.severity !== "medium")
                    return false;
            }
            return true;
        }), { numRuns: 100 });
    ...
    it("never throws for arbitrary snapshot inputs", () => {
        fc.assert(fc.property(fc.oneof(domSnapshotArb, fc.constant(null), fc.constant(undefined)), (snapshot) => {
            let threw = false;
            try {
                detectMixedContent("https://example.com/", snapshot);
            }
            catch {
                threw = true;
            }
...
//# sourceMappingURL=mixed-content.property.test.js.map