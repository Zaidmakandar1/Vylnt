...
function buildCookieHeader(
  name: string,
  value: string,
  attrs: { secure: boolean; httpOnly: boolean; sameSite: string | undefined }
): string {
  let header = `${name}=${value}`;
  if (attrs.httpOnly) header += "; HttpOnly";
  if (attrs.secure) header += "; Secure";
  if (attrs.sameSite !== undefined) header += `; SameSite=${attrs.sameSite}`;
  return header;
}
...
describe("Property 3: Cookie finding completeness", () => {
    ...
    fc.assert(
      fc.property(
        ...
        (name, value, attrs) => {
          ...
          const hasSecureFinding = findings.some(
            (f) =>
              f.type === "insecure_cookie" &&
              f.description.includes(name) &&
    ...
    fc.assert(
      fc.property(
        ...
        (name, value, attrs) => {
          ...
          const hasSecureCheckFinding = findings.some(
            (f) =>
              f.type === "insecure_cookie" &&
    ...
    fc.assert(
      fc.property(
        ...
        (url, name, value) => {
          ...
          const hasSameSiteNoneFinding = findings.some(
            (f) =>
              f.type === "insecure_cookie" &&
              f.severity === "high" &&
  ...
  it("produces zero findings for a fully-secured cookie on HTTPS", () => {
    fc.assert(
      fc.property(
        cookieNameArb,
        cookieValueArb,
        fc.oneof(fc.constant("Strict"), fc.constant("Lax")),
        (name, value, sameSite) => {
  ...
  it("each finding has a unique findingId (UUID-like) and required fields", () => {
    fc.assert(
      fc.property(
        fc.webUrl({ withQueryParameters: false, withFragments: false }),
        fc.array(
          fc.tuple(cookieNameArb, cookieValueArb, cookieAttrsArb).map(([n, v, a]) =>
            buildCookieHeader(n, v, a)
          ),
        ...
        (url, headers) => {
          ...
          for (const f of findings) {
            ...
            if (!f.affectedResource) return false;
          }
  ...
  it("never throws for arbitrary cookie header inputs", () => {
    fc.assert(
      fc.property(
        fc.webUrl({ withQueryParameters: false, withFragments: false }),
        fc.array(fc.oneof(fc.string(), fc.constant(""), fc.constant(";; ;"))),
        (url, headers) => {
          let threw = false;
          try {
