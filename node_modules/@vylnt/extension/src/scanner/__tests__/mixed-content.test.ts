...
const emptySnapshot: MixedContentDOMSnapshot = {
  scripts: [], links: [], images: [], iframes: [], media: [], formActions: [],
};

const allHttpsSnapshot: MixedContentDOMSnapshot = {
...
describe("detectMixedContent", () => {
  ...
  it("returns empty array for null snapshot without throwing", () => {
    expect(() => {
      const result = detectMixedContent("https://secure.example.com/", null as unknown as MixedContentDOMSnapshot);
  ...
  it("returns empty array for undefined snapshot without throwing", () => {
    expect(() => {
      const result = detectMixedContent("https://secure.example.com/", undefined as unknown as MixedContentDOMSnapshot);
      expect(result).toEqual([]);
    }).not.toThrow();
  });
  ...
  it("returns note finding for null pageUrl without throwing", () => {
    expect(() => {
      const result = detectMixedContent(null as unknown as string, emptySnapshot);
  ...
  it("correctly labels all resource types in details", () => {
    ...
    const findings = detectMixedContent("https://secure.example.com/", snapshot);
    const types = findings.map((f) => (f.details as { resourceType: string }).resourceType);
