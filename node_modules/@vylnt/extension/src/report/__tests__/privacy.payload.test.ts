...
const severityArb = fc.constantFrom(
  ...
  "low",
  "informational"
) as fc.Arbitrary<Finding["severity"]>;
...
const scanReportArb: fc.Arbitrary<ScanReport> = fc.record({
  ...
  mlFilterStatus: fc.constantFrom("applied", "unavailable", "not_applicable") as fc.Arbitrary<
    ScanReport["mlFilterStatus"]
  >,
});
...
describe("Property 22: Transmitted payload privacy invariant", () => {
  it("transmitted payload contains only ScanReport fields — no page content, DOM tree, or form data", async () => {
    await fc.assert(
      fc.asyncProperty(scanReportArb, async (report) => {
        ...
        expect((capturedBody as ScanReport).scannedUrl).toBe(report.scannedUrl);

        // The payload must contain findings metadata (array)
        expect(Array.isArray((capturedBody as ScanReport).findings)).toBe(true);

        // The payload must be a valid ScanReport structure (required fields present)
        const payload = capturedBody as ScanReport;
  ...
  it("payload does not grow beyond ScanReport fields even with many findings", async () => {
    await fc.assert(
      fc.asyncProperty(
        fc.array(findingArb, { minLength: 1, maxLength: 50 }),
        async (findings) => {
