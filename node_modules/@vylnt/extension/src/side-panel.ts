...
function renderFindings(findings: Finding[]): void {
  const list = document.getElementById("findings-list") as HTMLUListElement;
  const summary = document.getElementById("summary");

  if (!list) return;

  list.innerHTML = "";
  if (summary) summary.textContent = `Findings: ${findings.length}`;
...
function init(): void {
  const toggle = document.getElementById("scanning-toggle") as HTMLInputElement | null;
  const toggleText = document.getElementById("toggle-text");

  chrome.storage.sync.get([STORAGE_KEY_SCANNING_ENABLED], (result) => {
    const enabled =
      result[STORAGE_KEY_SCANNING_ENABLED] === undefined
        ? true
        : Boolean(result[STORAGE_KEY_SCANNING_ENABLED]);
    if (toggle) toggle.checked = enabled;
    if (toggleText) toggleText.textContent = enabled ? "Scanning: On" : "Scanning: Off";
  });

  if (toggle) {
    toggle.addEventListener("change", () => {
      const newEnabled = toggle.checked;
      if (toggleText) toggleText.textContent = newEnabled ? "Scanning: On" : "Scanning: Off";
      chrome.storage.sync.set({ [STORAGE_KEY_SCANNING_ENABLED]: newEnabled });
      chrome.runtime.sendMessage(
        { type: "SCANNING_STATE_CHANGED", enabled: newEnabled },
        () => {
  ...
  chrome.storage.onChanged.addListener((changes, areaName) => {
    if (areaName === "session" && changes.findings) {
      const findings = (changes.findings.newValue as Finding[]) || [];
      renderFindings(findings);
