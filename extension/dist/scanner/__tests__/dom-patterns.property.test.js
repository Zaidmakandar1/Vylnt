...
const patternTypeArb = fc.constantFrom("eval", "innerHTML", "document.write", "setTimeout_string", "setInterval_string");
/** Generates a script with at least one dangerous pattern of the given type. */
const scriptWithPatternArb = (patternType) => fc.record({
...
describe("Property 5: DOM pattern finding completeness", () => {
    it("produces at least one finding for each script containing a dangerous pattern", () => {
        fc.assert(fc.property(patternTypeArb, (patternType) => {
            ...
            return findings.some((f) => f.type === "dom_pattern" &&
                f.details.patternType === patternType);
        }), { numRuns: 200 });
    });
    it("produces no dom_pattern findings for scripts with no dangerous patterns", () => {
        fc.assert(fc.property(fc.array(safeScriptArb, { minLength: 0, maxLength: 5 }), (scripts) => {
    ...
    it("never throws for arbitrary script arrays", () => {
        const scriptArb = fc.record({
            sourceUrl: fc.oneof(fc.constantFrom("inline:0", "inline:1"), fc.webUrl()),
            content: fc.string(),
        });
        fc.assert(fc.property(fc.oneof(fc.array(scriptArb, { minLength: 0, maxLength: 5 }), fc.constant(null), fc.constant(undefined)), (scripts) => {
            let threw = false;
            try {
...
describe("Property 6: DOM pattern aggregation", () => {
    it("produces exactly one aggregated finding when > 20 instances of the same pattern exist", () => {
        fc.assert(fc.property(patternTypeArb, fc.integer({ min: 21, max: 50 }), (patternType, count) => {
            const snippet = dangerousSnippets[patternType];
            const content = (snippet + "\n").repeat(count);
            const scripts = [{ sourceUrl: "inline:0", content }];
            const findings = analyzeDOMPatterns("https://example.com/", scripts);
            const patternFindings = findings.filter((f) => f.type === "dom_pattern" &&
                f.details.patternType === patternType);
            if (patternFindings.length !== 1)
                return false;
            const details = patternFindings[0].details;
            return details.aggregated === true && details.count === count;
        }), { numRuns: 200 });
    });
    it("does NOT aggregate when there are exactly 20 or fewer instances", () => {
        fc.assert(fc.property(patternTypeArb, fc.integer({ min: 1, max: 20 }), (patternType, count) => {
            const snippet = dangerousSnippets[patternType];
            const content = (snippet + "\n").repeat(count);
            const scripts = [{ sourceUrl: "inline:0", content }];
            const findings = analyzeDOMPatterns("https://example.com/", scripts);
            ...
            return (patternFindings.length === count &&
                patternFindings.every((f) => !f.details.aggregated));
        }), { numRuns: 200 });
    });
    it("aggregates across multiple scripts for the same pattern type", () => {
        fc.assert(fc.property(patternTypeArb, fc.integer({ min: 11, max: 25 }), fc.integer({ min: 11, max: 25 }), (patternType, count1, count2) => {
            const snippet = dangerousSnippets[patternType];
            const scripts = [
...
//# sourceMappingURL=dom-patterns.property.test.js.map