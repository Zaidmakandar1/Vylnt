...
function makeCache(initial?: Map<string, { entries: CveEntry[]; timestamp: number }>): NvdCache {
  const store = initial ?? new Map();
  return { get: (k) => store.get(k), set: (k, v) => { store.set(k, v); } };
}

function makeLibrary(overrides: Partial<DetectedLibrary> = {}): DetectedLibrary {
...
function mockFetchOk(body = MOCK_CVE_RESPONSE): typeof fetch {
  return vi.fn().mockImplementation(async () => new Response(JSON.stringify(body), { status: 200 })) as unknown as typeof fetch;
}
...
describe("NvdClient.checkLibrary", () => {
  ...
  it("returns cached findings when fetch times out", async () => {
    ...
    const cachedEntries: CveEntry[] = [{ cveId: "CVE-2022-9999", cvssScore: 7.5, description: "Cached", detailUrl: "https://nvd.nist.gov/vuln/detail/CVE-2022-9999" }];
    const store = new Map([[`${lib.name}@${lib.version}`, { entries: cachedEntries, timestamp: Date.now() - 60 * 60 * 1000 }]]);
    const timeoutFetch = vi.fn().mockRejectedValue(Object.assign(new Error("aborted"), { name: "AbortError" })) as unknown as typeof fetch;
