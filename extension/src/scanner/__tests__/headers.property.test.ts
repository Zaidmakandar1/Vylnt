...
const VALID_VALUES: Record<RequiredHeader, string> = {
...
const missingHeadersArb = fc
  .subarray(REQUIRED_HEADERS as unknown as RequiredHeader[], { minLength: 1 })
  .map((arr) => new Set(arr));
...
describe("Property 1: Header finding completeness", () => {
  it("produces a finding for each missing required security header", () => {
    fc.assert(
      fc.property(urlArb, headersWithMissingArb, (url, { headers, missing }) => {
        ...
        if (missingFindings.length !== missing.size) return false;
    ...
    fc.assert(
      fc.property(urlArb, headersWithMissingArb, (url, { headers, missing }) => {
        ...
        for (const f of missingFindings) {
          if (!f.findingId) return false;
          if (!f.severity) return false;
          if (f.affectedResource !== url) return false;
  ...
  it("CSP and HSTS missing findings have severity 'high'", () => {
    fc.assert(
      fc.property(urlArb, (url) => {
        ...
        const hstsFinding = findings.find((f) =>
...
describe("Property 2: Scanner error resilience", () => {
  it("never throws for any arbitrary header array input", () => {
    ...
    const arbitraryHeaderArb = fc.array(
      fc.record({
        name: fc.oneof(fc.string(), fc.constant(undefined as unknown as string)),
        value: fc.oneof(fc.string(), fc.constant(undefined as unknown as string)),
      }),
      { maxLength: 20 }
    );

    fc.assert(
      fc.property(urlArb, arbitraryHeaderArb, (url, headers) => {
        let threw = false;
        try {
          inspectHeaders(url, headers as chrome.webRequest.HttpHeader[]);
        } catch {
          threw = true;
        }
        return !threw;
  ...
  it("returns an array (possibly empty) for null/undefined inputs without throwing", () => {
    ...
    const nullishArb = fc.oneof(
      ...
      fc.constant([]),
      fc.constant([null]),
      fc.constant([{ name: null, value: null }]),
      fc.constant([{ name: 42, value: {} }])
    );

    fc.assert(
      fc.property(urlArb, nullishArb, (url, headers) => {
        let result: unknown;
        let threw = false;
        try {
          result = inspectHeaders(url, headers as unknown as chrome.webRequest.HttpHeader[]);
        } catch {
          threw = true;
        }
