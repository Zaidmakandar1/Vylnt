...
import type { Finding, BrowserInfo, Severity, FindingType } from "../../types/index.js";
...
const arbNonDomFinding: fc.Arbitrary<Finding> = fc.record({
  findingId: fc.uuid(),
  type: arbFindingType,
...
const arbScannedUrl: fc.Arbitrary<string> = fc.webUrl();

// Fetch that always fails (ML filter unavailable) — keeps tests deterministic
function unavailableFetch(): typeof fetch {
  return vi.fn(async () => { throw new Error("unavailable"); }) as unknown as typeof fetch;
}
  ...
  it("every report has a UUID v4 sessionId", async () => {
    await fc.assert(
      fc.asyncProperty(
        arbBrowserInfo,
        arbScannedUrl,
        fc.array(arbNonDomFinding, { maxLength: 10 }),
        async (browserInfo, scannedUrl, nonDomFindings) => {
          ...
          expect(report.sessionId).toMatch(
  ...
  it("riskScore is always in [0, 100]", async () => {
    await fc.assert(
      fc.asyncProperty(
        arbBrowserInfo,
        arbScannedUrl,
        fc.array(arbNonDomFinding, { maxLength: 20 }),
        async (browserInfo, scannedUrl, nonDomFindings) => {
          const report = await buildReport({
            scannedUrl,
            browserInfo,
