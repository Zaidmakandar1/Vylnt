...
import type { Finding, BrowserInfo } from "../../types/index.js";
...
function makeDomFinding(id: string): Finding {
  return {
    findingId: id,
    type: "dom_pattern",
    severity: "medium",
...
function makeHeaderFinding(id: string): Finding {
  return {
    findingId: id,
    type: "missing_header",
    severity: "high",
...
function mockFetch(responses: Record<string, { classification: "safe" | "anomalous"; confidence: number }>): typeof fetch {
  return vi.fn(async (_url: RequestInfo | URL, init?: RequestInit) => {
    const body = JSON.parse((init?.body as string) ?? "{}");
    const findingId: string = body.finding_id;
    const resp = responses[findingId];
    if (!resp) throw new Error("unexpected finding_id");
    return new Response(
      JSON.stringify({ finding_id: findingId, ...resp }),
